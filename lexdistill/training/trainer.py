...
class BERTdotTrainer(HFTrainer):
    ...
    def _load_from_checkpoint(self, resume_from_checkpoint, model=None):
        ...
        logger.info("Loading model's weight from %s", resume_from_checkpoint)
        self.model.load_state_dict(
            ElectraModel.from_pretrained(resume_from_checkpoint).state_dict()
        )
...
class BERTcatTrainer(HFTrainer):
    ...
    def _load_from_checkpoint(self, resume_from_checkpoint, model=None):
        ...
        logger.info("Loading model's weight from %s", resume_from_checkpoint)
        self.model.load_state_dict(
            ElectraForSequenceClassification.from_pretrained(resume_from_checkpoint).state_dict()
        )
